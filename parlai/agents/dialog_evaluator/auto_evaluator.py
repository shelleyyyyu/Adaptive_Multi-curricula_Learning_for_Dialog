...
class TorchGeneratorWithDialogEvalAgent(TorchGeneratorAgent):
    ...
    def report(self):
        ...
        if self.metrics['human_response_length_cnt'] > 0:
            m['human_response_length'] = self.metrics['human_response_length'] / self.metrics['human_response_length_cnt']
        ...
        if self.metrics['human_sent_entropy_uni_cnt'] > 0:
            m['human_sent_entropy_uni'] = self.metrics['human_sent_entropy_uni'] / self.metrics['human_sent_entropy_uni_cnt']
        if self.metrics['human_sent_entropy_bi_cnt'] > 0:
            m['human_sent_entropy_bi'] = self.metrics['human_sent_entropy_bi'] / self.metrics['human_sent_entropy_bi_cnt']
        if self.metrics['human_sent_entropy_tri_cnt'] > 0:
            m['human_sent_entropy_tri'] = self.metrics['human_sent_entropy_tri'] / self.metrics['human_sent_entropy_tri_cnt']
        if self.metrics['human_word_entropy_uni_cnt'] > 0:
            m['human_word_entropy_uni'] = self.metrics['human_word_entropy_uni'] / self.metrics['human_word_entropy_uni_cnt']
        if self.metrics['human_word_entropy_bi_cnt'] > 0:
            m['human_word_entropy_bi'] = self.metrics['human_word_entropy_bi'] / self.metrics['human_word_entropy_bi_cnt']
        if self.metrics['human_word_entropy_tri_cnt'] > 0:
            m['human_word_entropy_tri'] = self.metrics['human_word_entropy_tri'] / self.metrics['human_word_entropy_tri_cnt']

        if not self.model.training:
            # TODO: add other metrics and balance these metrics
            m['total_metric'] = \
                (-base.get('ppl', 0) * 0.25) / 100 + \
                ...
                m.get('response_length', 0) / self.max_response_len + \
    ...
    def _eval_distinct_metrics(self, output, labels):
        ...
        for i in range(len(text)):
            ...
            self.metrics['intra_unigram'] += ((len(set(unigram_tokens)) + sys.float_info.epsilon) / (len(unigram_tokens) + sys.float_info.epsilon))
            self.metrics['intra_bigram_cnt'] += 1
            self.metrics['intra_bigram'] += ((len(set(bigram_tokens)) + sys.float_info.epsilon) / (len(bigram_tokens) + sys.float_info.epsilon))
            self.metrics['intra_trigram_cnt'] += 1
            self.metrics['intra_trigram'] += ((len(set(trigram_tokens)) + sys.float_info.epsilon) / (len(trigram_tokens) + sys.float_info.epsilon))
            ...
            self.metrics['human_intra_unigram'] += ((len(set(human_unigram_tokens)) + sys.float_info.epsilon) / (len(human_unigram_tokens) + sys.float_info.epsilon))
            self.metrics['human_intra_bigram_cnt'] += 1
            self.metrics['human_intra_bigram'] += ((len(set(human_bigram_tokens)) + sys.float_info.epsilon) / (len(human_bigram_tokens) + sys.float_info.epsilon))
            self.metrics['human_intra_trigram_cnt'] += 1
            self.metrics['human_intra_trigram'] += ((len(set(human_trigram_tokens)) + sys.float_info.epsilon) / (len(human_trigram_tokens) + sys.float_info.epsilon))
