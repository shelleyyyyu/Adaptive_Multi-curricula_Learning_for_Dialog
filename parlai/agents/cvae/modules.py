...
class LatentVariation(nn.Module):
    def __init__(self, input_size, latent_size, use_cuda=False):
    ...
    def forward(self, input_):
        ...
        mean = self.context_to_mean(input_)
        logvar = self.context_to_logvar(input_)
...
class CVAE(TorchGeneratorModel):
    ...
    def __init__(
            ...
            longest_label=1, latent_size=64, use_cuda=False
    ):
        """Initialize cvae model.

        See cmdline args in CvaeAgent for description of arguments.
        """
        super().__init__(
        ...
        rnn_class = CVAE.RNN_OPTS[rnn_class]
        ...
        self.latent_size = latent_size
        self.use_cuda = use_cuda
        self.priori = LatentVariation(hiddensize, self.latent_size, use_cuda)
        self.posterior = LatentVariation(hiddensize, self.latent_size, use_cuda)
        self.bow_project = nn.Sequential(
            nn.Linear(hiddensize + self.latent_size, 512),
            nn.Tanh(),
            nn.Dropout(0.2),
            nn.Linear(512, num_features)
        )
        self.init_dec_hidden = nn.Linear(latent_size, numlayers * hiddensize)
    ...
    def normal_kl_div(self, mean1, logvar1, mean2=None, logvar2=None):
        ...
        kl_div = 0.5 * torch.sum(
            logvar2 - logvar1 + (torch.exp(logvar1) + (mean1 - mean2).pow(2)) / torch.exp(logvar2) - 1.0, dim=1).mean().squeeze()
        return kl_div
...
class UnknownDropout(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.training and self.prob > 0:
            ...
            input = input.masked_fill(mask, self.unknown_idx)
        return input
    ...
    def forward(self, xes, hidden, attn_params):
        ...
        if self.attention == 'local':
            ...
            if attn_mask is not None:
                ...
                attn_w_premask.masked_fill_(~attn_mask.bool(), -NEAR_INF)
