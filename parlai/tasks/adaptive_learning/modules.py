...
class PolicyNet_Transformer(nn.Module):
    ...
    def forward(self, state, history_mean_emb):
        ...
        encoder_states = torch.sum(encoder_states, 2)
        # print('encoder_states', encoder_states.size()) # (10,64)
        encoder_states = self.history_ffn(encoder_states)
        # print('encoder_states', encoder_states.size()) # (10,5)
        encoder_states = torch.unsqueeze(torch.mean(encoder_states, dim=0), 0)
        # print('encoder_states', encoder_states.size()) # (1,5)
